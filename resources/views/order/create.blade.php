...
<div class="max-w-5xl mx-auto px-6 py-8 space-y-8">
    ...
    <div class="flex items-center gap-4 pb-6 border-b border-slate-200">
        <button type="button" id="backBtn" onclick="handleBack()"
            class="p-2 text-slate-500 hover:text-slate-800 hover:bg-slate-100 rounded-md transition shrink-0"
            title="Kembali">
        ...
        <div>
            <h1 class="text-2xl font-semibold text-[#0B2A4A] tracking-tight" id="stepTitle">Pilih Jenis Layanan</h1>
        </div>
    </div>

    @if ($errors->any())
        <div class="p-4 bg-rose-50 border border-rose-200 rounded-md text-xs text-rose-700">
            {{ $errors->first() }}
        </div>
    @endif
    ...
    <div id="step2" class="hidden space-y-4">
        <div class="border border-slate-200 rounded-md p-6">
            <h2 class="text-sm font-semibold text-[#0B2A4A] mb-4 pb-3 border-b border-slate-200" id="selectedCategoryTitle"></h2>
    ...
    <div id="step3" class="hidden grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div class="lg:col-span-8 border border-slate-200 rounded-md p-6 space-y-6">
            <div class="p-4 bg-slate-50 border border-slate-200 rounded-md">
                <span class="text-xs text-slate-500">Layanan Terpilih</span>
                <p class="text-sm font-semibold text-[#0B2A4A] mt-0.5" id="selectedProductLabel"></p>
            </div>
            ...
            <form action="{{ route('order.store') }}" method="POST" id="orderForm" class="space-y-5">
                ...
                <div class="space-y-1.5">
                    <label class="text-xs font-medium text-slate-700">Provinsi Lokasi Anda</label>
                    <select id="provinsiSelect"
                        class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-md text-xs focus:outline-none focus:bg-white focus:border-[#0B2A4A] transition">
                ...
                <div class="space-y-1.5">
                    <label class="text-xs font-medium text-slate-700">Cabang Penanggung Jawab</label>
                ...
                <div class="space-y-1.5">
                    <label class="text-xs font-medium text-slate-700">Catatan Pelanggan (Opsional)</label>
                    <textarea name="catatan_pelanggan" rows="3" placeholder="Tuliskan spesifikasi lokasi sampel atau instruksi khusus..."
                        class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-md text-xs focus:outline-none focus:bg-white focus:border-[#0B2A4A] transition">{{ old('catatan_pelanggan') }}</textarea>
                </div>

                <button type="submit" id="submitBtn" disabled
                    class="w-full py-3 bg-[#B8872F] hover:bg-[#9c7327] disabled:opacity-40 disabled:cursor-not-allowed text-white font-semibold text-xs rounded-md transition">
                    Kirim Pengajuan Order
                </button>
            </form>
        </div>

        <div class="lg:col-span-4 bg-slate-50 border border-slate-200 rounded-md p-5 h-fit text-xs space-y-3">
            <div class="font-semibold text-[#0B2A4A]">Informasi Pengajuan</div>
...
<script>
    ...
    function renderCategories() {
        ...
        if (!CATEGORIES || CATEGORIES.length === 0) {
            grid.innerHTML = `<div class="col-span-full border border-slate-200 rounded-md p-6 text-center">
                <p class="text-sm text-slate-500">Belum ada kategori layanan yang tersedia.</p>
                <p class="text-xs text-slate-400 mt-1">Jalankan <span class="font-mono-code">php artisan db:seed --class=ProductSeeder</span> untuk mengisi data layanan.</p>
            </div>`;
            return;
        }

        grid.innerHTML = CATEGORIES.map(cat => {
            const catName = cat.nama;
            const catDesc = cat.deskripsi ?? '';
            const prodCount = cat.products ? cat.products.length : 0;
            const iconMarkup = cat.icon ? `<span class="text-lg">${cat.icon}</span>` : defaultIcon;

            return `
                <div onclick="selectCategory(${cat.id})"
                    class="group border border-slate-200 hover:border-[#0B2A4A] rounded-md p-6 transition cursor-pointer flex flex-col justify-between">
                    <div>
                        <div class="w-10 h-10 bg-[#0B2A4A] rounded-md flex items-center justify-center mb-4">
                            ${iconMarkup}
                        </div>
                        <h3 class="font-semibold text-slate-800 text-sm mb-1.5">${catName}</h3>
                        <p class="text-xs text-slate-500 leading-relaxed">${catDesc}</p>
                    </div>
                    <div class="mt-5 pt-3 border-t border-slate-100 text-[11px] font-medium text-slate-400 group-hover:text-[#0B2A4A]">
                        ${prodCount} layanan tersedia
                    </div>
                </div>
            `;
        }).join('');
    ...
    function selectCategory(categoryId) {
        ...
        document.getElementById('selectedCategoryTitle').textContent = `Daftar Layanan — ${category.nama}`;
        ...
        if (products.length === 0) {
            ...
            list.innerHTML = products.map(p => {
                const pName = p.nama_produk;
                const pDesc = p.deskripsi;
                const pSatuan = p.satuan;

                return `
                    <div onclick="selectProduct(${p.id}, '${pName.replace(/'/g, "\\'")}')"
                        class="p-4 border border-slate-200 hover:border-[#0B2A4A] rounded-md cursor-pointer transition flex items-center justify-between group">
                        <div>
                            <p class="text-xs font-semibold text-slate-800 group-hover:text-[#0B2A4A]">${pName}</p>
                            ${pDesc ? `<p class="text-[11px] text-slate-500 mt-0.5 line-clamp-2">${pDesc}</p>` : ''}
                        </div>
                        ${pSatuan ? `<span class="text-[10px] font-medium text-slate-500 border border-slate-200 px-2 py-1 rounded shrink-0 ml-3">${pSatuan}</span>` : ''}
                    </div>
                `;
            }).join('');
        }
    ...
    function renderBranchOptions(provinsi) {
        ...
        container.innerHTML = withDistance.map((b, index) => {
            const bName = b.nama_cabang ?? b.nama;
            const bCity = b.kota;

            return `
                <label class="flex items-center justify-between p-3.5 border rounded-md cursor-pointer transition
                    ${index === 0 ? 'border-[#0B2A4A] bg-slate-50' : 'border-slate-200 hover:border-slate-300'}">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <p class="text-xs font-semibold text-slate-800">${bName}</p>
                            <p class="text-[11px] text-slate-500">${bCity}</p>
                        </div>
                    </div>
                    ${b.distance !== null ? `<span class="text-[10px] font-medium text-emerald-700">± ${b.distance.toFixed(1)} km</span>` : ''}
                </label>
            `;
        }).join('');
